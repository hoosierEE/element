...
bool Parser::eat(TokenEnum t) {bool ok = token_i < tokens.size() and t == tokens[token_i].type;
  token_i += ok;
  return ok;
}
...
bool Parser::expr() {return nve() or te() or true;}
...
bool Parser::noun() {// noun ::= term "[" Exprs "]" | "(" Exprs ")" | "{" Exprs "}" | Noun
  auto p = token_i;
  auto ok = term() and eat(LS) and exprs() and eat(RS);
  if (not ok) token_i = p;
  ok = eat(LP) and exprs() and eat(RP);
  if (not ok) token_i = p;
  ok = eat(LC) and exprs() and eat(RC);
  if (not ok) token_i = p;
  ok = Noun();
  if (not ok) token_i = p;
  return ok;
...
bool Parser::Verb() {
  auto p = token_i;
  auto ok = match({DASH, COLON, PLUS, STAR, PERCENT, BANG, AMP, VERT, LESS,
     MORE, EQUAL, TILDE, COMMA, CARET, HASH, UNDERBAR, DOLLAR, HUH, AT, DOT});
  if (ok) {
    int p1 = p;
